...
type SuaveRuntime interface {
	add(c uint64, d uint64) (uint64, error)
	...
	naclEncrypt(publicKey string, message []byte) ([]byte, error)
...
func (b *SuaveRuntimeAdapter) add(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["add"].Inputs.Unpack(input)
	if err != nil {
	...
	if output1, err = b.impl.add(c, d); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["add"].Outputs.Pack(output1)
	if err != nil {
...
func (b *SuaveRuntimeAdapter) naclEncrypt(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["naclEncrypt"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if encrypted, err = b.impl.naclEncrypt(publicKey, message); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["naclEncrypt"].Outputs.Pack(encrypted)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
